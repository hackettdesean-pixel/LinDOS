...
class LinDOSDesktop:
    ...
    def create_main_layout(self):
        ...
        apps = [
            ...
            ("Run Optimizer", "Execute hardware daemon", self.run_optimizer),
    ...
    def run_optimizer(self):
        try:
            if os.path.exists("./bin/lindos-optimizer"):
                result = subprocess.run(["./bin/lindos-optimizer"], capture_output=True, text=True)
                output = result.stdout if result.returncode == 0 else result.stderr
            else:
                output = "Optimizer binary not found. Running fallback memory compaction..."
    ...
    def open_settings(self):
        ...
        settings_win.title("LinDOS Settings")
        settings_win.geometry("400x300")
        settings_win.configure(bg=self.bg_color)

        lbl = tk.Label(settings_win, text="LinDOS System Controls", fg=self.text_color, bg=self.bg_color, font=("Segoe UI", 12, "bold"))
        lbl.pack(pady=15)

        def check_zram():
            try:
                result = subprocess.run(["swapon", "--show"], capture_output=True, text=True)
                output = result.stdout if result.stdout else "No active swap found."
                messagebox.showinfo("ZRAM Swap Status", output)
            except Exception as e:
                messagebox.showerror("Error", str(e))

        zram_btn = tk.Button(settings_win, text="Check Active Swap (ZRAM)", command=check_zram, bg=self.card_bg, fg=self.accent_color, font=("Segoe UI", 9, "bold"), bd=0, padx=10, pady=8)
        zram_btn.pack(pady=10, fill=tk.X, padx=20)
        
        close_btn = tk.Button(settings_win, text="Close", command=settings_win.destroy, bg="#292e42", fg=self.text_color, font=("Segoe UI", 9), bd=0, padx=10, pady=5)
    ...
    def check_system(self):
        try:
            if os.path.exists("/proc/meminfo"):
                ...
                meminfo = "System memory stats unavailable in current environment."
